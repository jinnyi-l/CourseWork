...
buildDLX bitmap rows cols = do
    -- Create the root header node (head of the DLX matrix)
    head <- newDNode

    -- Create an array of column headers (one for each column)
    columnHeaders <- forM [0..cols-1] $ \_ -> newDNode
    ...
    forM_ (zip [0..] bitmap) $ \(i, row) -> do
        prevNodeRef <- newSTRef Nothing -- Previous node in the current row
        firstNodeRef <- newSTRef Nothing -- First node in the current row (for circular linking)
        ...
        forM_ (zip [0..] row) $ \(j, value) -> do
            when value $ do
                ...
                case colDown of
                    Just downNode -> do
                        setAttr down newNode (Just downNode)
                        setAttr up downNode (Just newNode)
                    Nothing -> return () -- The first node in this column

                setAttr ctl newNode colHeader
                colSize <- getAttr size colHeader
                setAttr size colHeader (colSize + 1)
                prev <- readSTRef prevNodeRef
                case prev of
                    Just prevNode -> do
                        setAttr right prevNode (Just newNode)
                        setAttr left newNode (Just prevNode)
                    Nothing -> do
                        writeSTRef firstNodeRef (Just newNode)
                writeSTRef prevNodeRef (Just newNode)

        firstNode <- readSTRef firstNodeRef
        lastNode <- readSTRef prevNodeRef
        case (firstNode, lastNode) of
            (Just firstN, Just lastN) -> do
                setAttr left firstN (Just lastN)
                setAttr right lastN (Just firstN)
            _ -> return () -- No nodes in this row
